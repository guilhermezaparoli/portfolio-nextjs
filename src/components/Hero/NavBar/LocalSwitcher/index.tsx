...
console.log(router, "aaa")
    ...
    return (
            <>
                <button onClick={() => onSelectChange("pt-BR")} className={`flex items-center p-2 w-full ${selected == "en"  && "brightness-50"}` }>
                    <Image src={brasil} alt="Portuguese" width={30} height={15} />
                </button>
                <button onClick={() => onSelectChange("en")} className={`flex items-center p-2 w-full ${selected == "pt-BR"  && "brightness-50"}`}>
